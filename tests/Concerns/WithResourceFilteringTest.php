...
describe('WithResourceFiltering', function (): void {
    test('applyFilters correctly modifies the query', function (): void {
        ...
        $originalQuery = invade($this->resourceController)->getModelQuery();
        $newQuery = invade($this->resourceController)->applyFilters(clone $originalQuery);
    ...
    test('getFilters returns resource filters', function (): void {
        $filters = invade($this->resourceController)->getFilters();
